...
Effect<YuePaoReportState> buildEffect() {
  return combineEffects(<Object, Effect<YuePaoReportState>>{
...
void _init(Action action, Context<YuePaoReportState> ctx) {
  _isClickReport = false;
}

// 举报
void _onReport(Action action, Context<YuePaoReportState> ctx) async {
  ...
  try {
    ...
    await netManager.client
        .postFeedBack(ctx.state.id, ctx.state.type, picList, content);
...
void _onSelectPic(Action action, Context<YuePaoReportState> ctx) async {
  var list =
...
Future<List<String>> _pickImg(
    Context<YuePaoReportState> ctx, Action action, int needCount) async {
  List<String> ret = [];
