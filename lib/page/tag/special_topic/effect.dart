...
Effect<SpecialTopicState> buildEffect() {
  return combineEffects(<Object, Effect<SpecialTopicState>>{
    Lifecycle.initState: _init,
    SpecialTopicAction.tagClick: _onTagPage,
    SpecialTopicAction.loadData: _onLoadData,
    SpecialTopicAction.loadMoreData: _onLoadMoreData,
  });
}

///初始化数据
Future _init(Action action, Context<SpecialTopicState> ctx) async {
  Future.delayed(Duration(milliseconds: 200), () async {
    _onLoadData(action, ctx);
    //var list = await getAdsByType(AdsType.searchType);
   // ctx.dispatch(SpecialTopicActionCreator.setAds(list ?? []));
    //eaglePage(ctx.state.selfId(), sourceId: ctx.state.eagleId(ctx.context));
  });
}

///初始化数据
Future _onLoadData(Action action, Context<SpecialTopicState> ctx) async {
  try {
    var specialModel = await netManager.client.getGroup(1, ctx.state.pageSize);
    ctx.dispatch(SpecialTopicActionCreator.setLoadData(specialModel.list));
...
///初始化数据
Future _onLoadMoreData(Action action, Context<SpecialTopicState> ctx) async {
  try {
    var number = ctx.state.pageNumber + 1;
    var specialModel =
        await netManager.client.getGroup(number, ctx.state.pageSize);
    ctx.dispatch(SpecialTopicActionCreator.setLoadMoreData(specialModel.list));
...
///进入标签页
_onTagPage(Action action, Context<SpecialTopicState> ctx) {
