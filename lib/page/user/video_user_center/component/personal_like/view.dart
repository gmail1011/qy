...
Widget buildView(
    UserLikePostState state, Dispatch dispatch, ViewService viewService) {
  ...
  return NotificationListener(
    ...
    child: !state.loadComplete
        ...
        : EasyRefresh.custom(
            ...
            topBouncing: false,
            emptyWidget:
                state.videoList.length != 0 ? null : EmptyWidget('user', 2),
            footer: LoadMoreFooter(hasNext: state.hasNext ?? false),
            onLoad: () async {
              if (state.hasNext ?? false) {
                dispatch(UserLikePostActionCreator.onLoadData());
              }
            },
            slivers: <Widget>[
              SliverGrid(
