              ...
              children: <Widget>[
                ...
                Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        "${state.configData?.gameTax ?? ""}% 本次提现手续：${(state.handlingFee ?? 0).toStringAsFixed(2)}  实际到账金额：${(state.actualAmount ?? 0).toStringAsFixed(2)}",
                        style: TextStyle(
                          color: AppColors.withdrawsubTextColor,
                          fontSize: Dimens.pt12,
                        ),
...
String _getPayMoneyRange(WithDrawState state) {
  try {
    int minMoney =
        state.configData?.channels[state.withdrawType]?.qpMinMoney ?? 0;
    int maxMoney =
        state.configData?.channels[state.withdrawType]?.qpMaxMoney ?? 0;
