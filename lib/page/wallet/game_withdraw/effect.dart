...
void _initData(Action action, Context<WithDrawState> ctx) async {
  ...
  Future.delayed(Duration(milliseconds: 200), () async {
    ...
    ctx.dispatch(WithDrawActionCreator.refreshAmount(
...
void _commonWithdrawReq(Context<WithDrawState> ctx, bool isbankType,
  ...
  try {
    ...
    int incomeMoneyYuan = Config?.gameBalanceEntity?.wlTransferable ?? 0;
    ...
    int minMoneyFen =
        ctx.state.configData?.channels[ctx.state.withdrawType].qpMinMoney ?? 0;
    ...
    int maxMoneyFen =
        ctx.state.configData?.channels[ctx.state.withdrawType].qpMaxMoney ?? 0;
