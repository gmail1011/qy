...
class PayForNakeChatState with EagleHelper implements Cloneable<PayForNakeChatState> {
  ...
  PayForNakeChatState({PayForNakeArgs args}) {
    if (args != null) {
      ///代充信息
      ...
      if (args.rechargeModel != null) {
        ...
        payMoney = args.vipitem.discountedPrice * 10;
      } else {
        payMoney = 0;
      }
...
PayForNakeChatState initState(PayForNakeArgs args) {
  PayForNakeChatState newState = PayForNakeChatState(args: args);
  return newState;
}

class PayForNakeArgs {
  DCModel dcModel;
  RechargeTypeModel rechargeModel;
  ProductItemBean vipitem;
  bool isDialog = true;
