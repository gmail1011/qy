...
class MyFansPageState extends State<MyFansPage> {
  ...
  void initData() async {
    try {
      FansObj fansObjTemp = await netManager.client.getBloggerFansList(
        pageNumber,
        pageSize,
        widget.uid,
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: fansObj == null
          ? LoadingWidget()
          : fansObj.list == null || fansObj.list.length == 0
              ? CErrorWidget("暂无数据")
              : Container(
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  Visibility(
                                      visible:
                                          fansObj.list[index].hasFollow ?? false
                                              ? false
                                              : true,
                                      child: GestureDetector(
                                        onTap: () async {

                                          if(fansObj.list[index].uid == GlobalStore.getMe().uid){
                              ...
                              Container(
                                height: 1.w,
                                margin:
                                    EdgeInsets.only(left: 78.w, right: 58.w),
