...
func (g *Generator) generate() {
	g.P("part of 'events.dart';")
	g.P("")
	g.P("class EventHelper {")
	g.P("  static void fireCompleter(")
	g.P("    Completer completer,")
	g.P("    Session session,")
	g.P("    pkt.Payload payload,")
	g.P("  ) {")
	g.P("    switch (payload.op) {")
...
func (g *Generator) generateEnum(enum *EnumDescriptor) {
	...
	for _, ev := range enum.Value {
		...
		if strings.HasPrefix(name, "EnumS2C") {
			funcName := strings.TrimLeft(name, "Enum")
			g.P(fmt.Sprintf("case pkt.OType.%s:", name))
			g.P(fmt.Sprintf("        completer.complete(PacketEvent<pkt.%s>(session, payload.%s,));", funcName, strings.ToLower(funcName[:1])+funcName[1:]))
			g.P("        return;")
		}
	}
