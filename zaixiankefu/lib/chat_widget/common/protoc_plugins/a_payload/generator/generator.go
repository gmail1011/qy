...
func (g *Generator) generate() {
	...
	g.P("")
	g.P("import (")
	g.P(`	proto "github.com/golang/protobuf/proto"`)
	g.P(")")
	g.P()
	g.P("// NewPayload for proto")
	g.P("func NewPayload(v proto.Message) *Payload {")
	g.P("switch vv := v.(type) {")
...
func (g *Generator) generateEnum(enum *EnumDescriptor) {
	...
	for _, ev := range enum.Value {
		...
		if strings.HasPrefix(name, "Enum") {
			...
			g.P(fmt.Sprintf("	case *%s:", funcName))
			g.P(fmt.Sprintf("return &Payload{Op: OType_%s, Pkt: &Payload_%s{%s: vv}}", name, funcName, funcName))
		}
	}
