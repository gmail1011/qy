...
func (d *FileDescriptor) goFileName(pathType pathType) string {
	...
	// }
	name := "payload_dispatcher_initializater.dart"
...
func (g *Generator) generate() {
	...
	g.P("")
	g.P("void _initPayloadDispatcher(_PaylaodDispatcher d) {")
...
func (g *Generator) generateEnum(enum *EnumDescriptor) {
	...
	for _, ev := range enum.Value {
		...
		if strings.HasPrefix(name, "EnumS2C") {
			...
			g.P(fmt.Sprintf("  d.add(OType.%s.value,", name))
			g.P(fmt.Sprintf("      (s, p) => eventMgr.fire(PacketEvent(s, p.%s)),);", strings.ToLower(funcName[:1])+funcName[1:]))
		}
		if strings.HasPrefix(name, "EnumUpdate") {
			funcName := strings.TrimLeft(name, "Enum")
			g.P(fmt.Sprintf("  d.add(OType.%s.value,", name))
			g.P(fmt.Sprintf("      (s, p) => eventMgr.fire(PacketEvent(s, p.%s)),);", strings.ToLower(funcName[:1])+funcName[1:]))
		}
