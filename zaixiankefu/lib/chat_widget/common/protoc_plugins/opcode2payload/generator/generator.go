...
// 	}
// }
func (g *Generator) generatePacketPayloads(file *FileDescriptor) {
	g.file = file
	// 代码生成块
	for _, e := range g.file.enum {
		if e.GetName() != "OType" {
			continue
		}
		...
		for _, ev := range e.Value {
			name := ev.GetName()
			if strings.HasPrefix(name, "OType") {
				if strings.HasSuffix(name, "Start") {
					g.P("")
				}
				g.P(fmt.Sprintf("    // %s", name))
			...
			if strings.HasPrefix(name, "Enum") {
				name = strings.TrimLeft(name, "Enum")
			}

			g.P(fmt.Sprintf("    %s %s = %d;",
				name,
				toCaseConvert(name),
				ev.GetNumber()))
		}
...
func (g *Generator) generateImportPortos() {
	inpath := g.Request.GetParameter()
	dirs := make([]string, 0, 100)
	dirs = append(dirs, inpath)
	for i := 0; i < len(dirs); i++ {
		...
		for _, file := range files {
			fileName := file.Name()
			if file.IsDir() {
				dirs = append(dirs, d+"/"+fileName)
				continue
			}
			if strings.HasSuffix(fileName, ".proto") &&
				(!strings.HasPrefix(fileName, "a_")) &&
				(!strings.HasPrefix(fileName, "ac_")) &&
				(!strings.HasPrefix(fileName, "as_")) &&
				(!strings.HasSuffix(fileName, "_micro.proto")) {
				g.P("import " + "\"" + fileName + "\";")
			}
		}
	}
...
func (g *Generator) GenerateAllFiles() {
	...
	for _, file := range g.allFiles {
		...
		//加载模版文件
		inPath := g.Request.GetParameter()
		fileTemplate, err := os.Open(inPath + "/a_payload.proto.template")
		if err != nil {
		...
		for {
			...
			if strings.Contains(lines, "/*import_protos*/") {
				g.generateImportPortos()
			} else if strings.Contains(lines, "/*packet_payloads*/") {
				g.generatePacketPayloads(file)
			} else {
				g.P(lines)
			}
			lines = ""
		...
		g.Response.File = append(g.Response.File, &plugin.CodeGeneratorResponse_File{
			Name:    proto.String("a_payload.proto"),
