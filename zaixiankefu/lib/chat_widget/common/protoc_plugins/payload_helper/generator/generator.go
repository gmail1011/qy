...
func (g *Generator) generate() {
	...
	g.P("")
	g.P("Payload _createPayload<T extends GeneratedMessage>(T req) {")
...
func (g *Generator) generateEnum(enum *EnumDescriptor) {
	...
	for _, ev := range enum.Value {
		...
		if strings.HasPrefix(name, "Enum") {
			...
			g.P(fmt.Sprintf("  if (req is %s) {", funcName))
			g.P("    var p = Payload();")
			g.P(fmt.Sprintf("    p.op = OType.%s;", name))
			g.P(fmt.Sprintf("    p.%s = req;", strings.ToLower(funcName[:1])+funcName[1:]))
			g.P("    return p;")
			g.P("}")
		}
	}
