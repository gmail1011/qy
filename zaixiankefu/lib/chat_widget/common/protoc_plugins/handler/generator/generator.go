...
// GeneratTemplate 生成模版文件部分
func (g *Generator) GeneratTemplate(fileName string) {

	inPath := g.Request.GetParameter()
	inPaths := strings.Split(inPath, ",")
	if len(inPaths) < 2 {
		g.Fail("Missing parameters !")
	}
	g.compareFilePath = inPaths[1]
	inPath = inPaths[0]
	...
	opcodeFile, err := os.Open(inPath + "object_type.proto")
	if err != nil {
		g.Fail("Failed to open object_type.proto file！")
	}
	defer opcodeFile.Close()
	b := bufio.NewReader(opcodeFile)
	opfLine := ""
	opcodeFlg := false
	for {
		...
		opfLine = strings.TrimSpace(opfLine)
		if opcodeFlg && strings.HasPrefix(opfLine, "Enum") {
			name := strings.TrimPrefix(opfLine, "Enum")
			if !strings.Contains(name, "=") {
				g.Fail("Enum not find '='")
			}

			name = strings.TrimSpace(strings.Split(name, "=")[0])
			enums[name] = true
		}
		if strings.Contains(opfLine, "enum OType {") && !opcodeFlg {
			opcodeFlg = true
		}
		if opfLine == "}" && opcodeFlg {
			opcodeFlg = false
		}
		opfLine = ""
	}

	fileTemplate, err := os.Open(inPath + "a_proto.handler.template")
	if err != nil {
		g.Fail("Failed to open template file！")
	}
	defer fileTemplate.Close()
	br := bufio.NewReader(fileTemplate)
	...
	for {
		...
		t := strings.TrimSpace(lines)

		if imflg > 0 && lines != "" {
			g.file.importStr[t] = false
		}
		if strings.HasPrefix(t, "import") && strings.HasSuffix(t, "(") && imflg == 0 {
			imflg = 1
		}
		if strings.HasSuffix(t, ")") {
			if imflg == 1 {
				for k, v := range g.file.importStr {
					if v {
						g.P(k)
					}
				}
		...
		if strings.Contains(lines, "/*handler_message*/") {
			for _, desc := range g.file.desc {
				// Don't generate virtual messages for maps.
				if desc.GetOptions().GetMapEntry() {
					continue
				}
				g.generateMessage(desc, enums)
			}
...
// GenerateMessage message生成块
func (g *Generator) generateMessage(message *Descriptor, enums map[string]bool) {

	typeName := message.TypeName()
	tLen := len(typeName)
	if tLen <= 0 {
		g.Fail("Parsing message Name failed!")
	} else if tLen > 1 {
		// 遍历发现2个以上的message 包含关系不做处理
		return
	}
	// 过滤掉S2C的 开头的函数 和 OType 没有定义的 Enum
	if strings.HasPrefix(typeName[0], "S2C") || !enums[typeName[0]] {
		return
	}
	g.addInitf("	registerHandler(pkt.OType_Enum%s, handler%s)", typeName[0], typeName[0])

	funcName := "handler" + typeName[0]
	...
	if oneFuncStrs != nil {
		...
		// 生成Handler函数
		g.P(fmt.Sprintf("// handler%s handler for %s", typeName[0], typeName[0]))
		g.P(fmt.Sprintf("func handler%s(ctx *MessageContext, msg *pkt.Payload) error {", typeName[0]))
		g.P(fmt.Sprintf("	req := msg.Get%s()", typeName[0]))
		g.P(fmt.Sprintf(`	log.Error("not impl opcode: %%d, %%+v", pkt.OType_Enum%s, req)`, typeName[0]))
		g.P("	return nil")
		g.P("}")
		g.P()
	}
...
func (d *FileDescriptor) getFileName(pathType pathType) string {
	name := *d.Name
	if ext := path.Ext(name); ext == ".proto" || ext == ".protodevel" {
		name = name[:len(name)-len(ext)]
	}
	name = "handler_" + name + ".go"
...
func getFuncNmae(funcStr string) (res string) {
	if len(funcStr) < 5 || !strings.HasPrefix(funcStr, "func ") {
		return
	}
	res = strings.Split(strings.TrimLeft(funcStr, "func "), "(")[0]
	return
}
...
func (g *Generator) handlerCompareFile(fileName string) {
	...
	g.file.importStr = make(map[string]bool)
	...
	for {
		...
		if strings.HasPrefix(lines, "func handler") && strings.HasSuffix(lines, "{") {
			imflg = 0
			flg++
			funcName = getFuncNmae(lines)
...
func (g *Generator) GenerateAllFiles() {
	...
	for _, file := range g.allFiles {
		...
		//加载模版文件内容
		fileName := file.getFileName(g.pathType)
		g.GeneratTemplate(fileName)
